...
**STATUS: shoalfang ✅ / silt-lurcher ✅ / barotaur ✅ BUILT (+ decoy buoy & Launch button ✅). REMAINING TO BUILD: anglerlure, chorus, tidehulk** — each needs a novel mechanic (render disguise / false-sonar injection / current push), so give each its own verified commit. Pattern to copy: spawnCreature trait branch → tickX() → creatureTick dispatch → render colour+glyph → contact-line `what` → creature.test.js block → battery → commit.
